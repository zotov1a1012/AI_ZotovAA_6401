...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Backprop through second layer affine
        da1, dw2, db2 = affine_backward(dscores, cache2)
        grads['W2'] = dw2 + self.reg * self.params['W2']
        grads['b2'] = db2

        # Backprop through first layer affine_relu
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # Dimensions: input_dim -> hidden_dims[0] -> ... -> hidden_dims[-1] -> num_classes
    ...
    def loss(self, X, y=None):
        ...
        caches = []
        out = X
        # Forward pass for each hidden layer
        for i in range(self.num_layers - 1):
            w = self.params['W%d' % (i+1)]
            b = self.params['b%d' % (i+1)]
            # Affine forward
            a, cache_affine = affine_forward(out, w, b)
            # Batch normalization (if applicable)
            if self.normalization == 'batchnorm':
                gamma = self.params['gamma%d' % (i+1)]
                beta = self.params['beta%d' % (i+1)]
                bn_param = self.bn_params[i]
                a, cache_bn = batchnorm_forward(a, gamma, beta, bn_param)
            else:
                cache_bn = None
            # ReLU
            ...
            caches.append((cache_affine, cache_bn, cache_relu, cache_dropout))
            out = a

        # Last layer (affine, no ReLU, no norm, no dropout)
        ...
        caches.append((cache_last, None, None, None))  # last layer has no bn/relu/dropout
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # Backward pass through last layer
        dout, dw_last, db_last = affine_backward(dscores, caches[-1][0])
