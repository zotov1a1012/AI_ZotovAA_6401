...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, sample_mean, sample_var, x_centered, std, x_norm, gamma, beta, eps)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, sample_mean, sample_var, x_centered, std, x_norm, gamma, beta, eps = cache
    N, D = x.shape
    ...
    dmean = np.sum(dx_norm * (-1.0 / std), axis=0) + dvar * np.mean(-2.0 * x_centered, axis=0)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_reshaped, cache_reshaped = batchnorm_forward(x_reshaped, gamma, beta, bn_param)
    out = out_reshaped.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    cache = (x, gamma, beta, bn_param, cache_reshaped)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, beta, bn_param, cache_reshaped = cache
    N, C, H, W = x.shape
    dout_reshaped = dout.transpose(0, 2, 3, 1).reshape(-1, C)
    dx_reshaped, dgamma, dbeta = batchnorm_backward(dout_reshaped, cache_reshaped)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # Flatten spatial and intra-group dimensions: (N*G, (C//G)*H*W)
    ...
    # Reshape back
    x_norm = x_norm_flat.reshape(N, G, C // G, H, W)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    _, C, _, _ = x.shape
    Cg = C // G
    dx_norm_reshaped = dx_norm.reshape(N, G, Cg, H, W)
    x_reshaped = x.reshape(N, G, Cg, H, W)

    # Flatten spatial dimensions
    dx_norm_flat = dx_norm_reshaped.reshape(N * G, -1)
    x_flat = x_reshaped.reshape(N * G, -1)

    # Statistics from forward pass
    mean_flat = mean.reshape(N * G, 1)
    var_flat = var.reshape(N * G, 1)

    # Compute gradients for group normalization (similar to batchnorm but without gamma/beta)
    N_total = dx_norm_flat.shape[1]  # D = (C//G)*H*W
    dvar = np.sum(dx_norm_flat * (x_flat - mean_flat) * (-0.5) * (var_flat + eps)**(-1.5), axis=1, keepdims=True)
    dmean = np.sum(dx_norm_flat * (-1.0 / np.sqrt(var_flat + eps)), axis=1, keepdims=True) + dvar * np.mean(-2.0 * (x_flat - mean_flat), axis=1, keepdims=True)
    dx_flat = dx_norm_flat / np.sqrt(var_flat + eps) + dvar * (2.0 * (x_flat - mean_flat) / N_total) + dmean / N_total
